...
if (isset($_GET['tec_id'])) {
	...
	if(!empty($tokenFiter))
	{
		$characterCertificateData = $this->db->query("SELECT * FROM ".Table::studentBonafideCertificate." WHERE id = '{$tokenFiter['insertId']}' AND status = '1' AND schoolUniqueCode = '{$tokenFiter['schoolUniqueCode']}' LIMIT 1")->result_array()[0];
	...
	if (empty($salaryDetails)) {
		$msgArr = [
			'class' => 'danger',
			'msg' => 'Bonafide Certificate Details Not Found.',
		];
	...
	$msgArr = [
		'class' => 'danger',
		'msg' => 'Bonafide Certificate Not Found.',
	];
	$this->session->set_userdata($msgArr);
	...
	<style>
		...
		@media print {
			...
			#contentData p {
				...
				text-align:justify;font-size:18px;margin:40px;margin-bottom:5px;line-height:40px;
			}

			#moreCT p {
				text-align:justify;font-size:18px;margin:40px;margin-top:5px;line-height:40px;
			}
...
<body>
	...
	<title>Digital Bonafide Certificate - digitalfied.com</title>
	<!--Generating Custom Title For Page-->
	...
	<div class="container border" >
		...
		<table>
			<tr>
				...
				<td>
					...
					<img class="qrcode" src="https://chart.googleapis.com/chart?chs=150x150&amp;cht=qr&amp;chl=<?= base_url('bonafideCertificate?tec_id=') . $_GET['tec_id'] ?>&amp;choe=UTF-8" alt="QR code" width="130px" style="padding-left:20px;"/> </br>
					<h4>
		...
		<h4 style="font-size: 36px; font-weight:bold; margin-bottom:20px;">
			<center><i>Bonafide Certificate</i></center>
		</h4>
